...
SELECT * from uci_credit_card limit 10 ;
select count(*) as total_clients from uci_credit_card;
...
CHANGE `default.payment.next.month` default_next_month INT;

-- ============================================
-- STEP 3: VERIFY RENAME WORKED
-- ============================================

SELECT default_next_month FROM uci_credit_card LIMIT 5;
...
COUNT(*) AS total_clients,
SUM(default_next_month) AS total_defaults,
ROUND(SUM(default_next_month) * 100/COUNT(*),2) AS default_rate_pct,
...
risk_tier , 
age_band , 
CASE EDUCATION WHEN 1 THEN 'Graduate' 
...
COUNT(*) AS total_clients ,
SUM(default_next_month) AS total_defaults ,
ROUND(SUM(default_next_month)*100/COUNT(*) , 0 ) AS default_rate_pct ,
ROUND(SUM(LIMIT_BAL),0) AS total_exposure 
FROM uci_credit_card
...
COUNT(*) AS total_clients ,
ROUND(SUM(LIMIT_BAL),0) AS total_exposure,
ROUND(SUM(LIMIT_BAL) * 100/COUNT(*) OVER() , 2) AS exposure_rate_pct
FROM uci_credit_card
GROUP BY risk_tier 
)
SELECT * FROM exposure_summary 
ORDER BY total_exposure DESC ; 

-- ============================================
-- STEP 16: CREATE FINAL CLEAN EXPORT TABLE
-- This feeds directly into Python and Power BI
-- ============================================
...
SELECT COUNT(*) AS total_rows FROM credit_risk_clean;
